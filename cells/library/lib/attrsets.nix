...
let
  ...
  inherit (l) nameValuePair genAttrs hasAttr;
in
rec {
  ...
  filterDerivations = filterAttrsOnlyRecursive (
    n: attrs: l.isDerivation attrs || attrs.recurseForDerivations or false
  );
          ...
          if pred name v then
            [
              (l.nameValuePair name (
                if l.isAttrs v && !l.isDerivation v then filterAttrsOnlyRecursive pred v else v
              ))
            ]
          else
            [ ]
  ...
  recursiveMergeAttrsWithNames =
    names: f: l.zipAttrsWithNames names (name: l.foldl' f { });

  recursiveMergeAttrsWith =
    f: sets: recursiveMergeAttrsWithNames (l.concatMap l.attrNames sets) f sets;
  ...
  */
  ...
  mapNestedMattr =
    let
      f =
        x:
        l.foldr (n: acc: acc // l.mapAttrs' (n': l.nameValuePair (n + "-" + n')) x.${n})
          { }
          (l.attrNames x);
    in
    f;
