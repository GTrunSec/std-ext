...
let
        ...
        (
          ...
          l.mapAttrs (
            ...
            (l.pipe blocks [
              ...
              (l.mapAttrs (
                name: value:
                if value ? process-compose then
                  {
                    name = "//${user}/${cellBlock}/${name}";
                    value =
                      let
                        extraAttrPath =
                          if l.hasAttr "extraAttrPath" value.process-compose then
                            value.process-compose.extraAttrPath
                          else
                            [ ]
                        ;
                      in
  ...
  composeYaml = writeConfig "compose.yaml" (
    { processes = l.foldr (a: b: a // b) { } (map cellTasks cellBlocks); } // attrs
  );
in
